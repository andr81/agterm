...
**A host that forbids remote forwarding gets no statuses.** The tab still opens, after three quick failed attempts, a probe connection and a line saying the forward was refused, and the row stays idle for that attachment. Ask the host's admin about `AllowTcpForwarding` and `PermitListen` if you want the bridge there.
