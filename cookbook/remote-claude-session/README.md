...
It asks for the token without echoing it and stores it on the host in `~/.agt-remote/env` as `CLAUDE_CODE_OAUTH_TOKEN`, mode 600. Every tmux session `attach` creates gets it in its own environment, whatever the login shell and whichever startup files it reads; `~/.profile` sources the file as well, for a shell you open on the host by hand. Run it again to replace the token. Signing in on the host interactively instead, with `claude` and `/login`, works the same way and needs no token.
